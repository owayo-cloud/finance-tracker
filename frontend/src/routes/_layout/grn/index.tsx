...
import type { GRNCreate, SupplierPublic, TransporterPublic, ProductPublic, ProductsPublic, TransporterCreate, SupplierCreate } from "@/client";
import useCustomToast from "@/hooks/useCustomToast";

// Utility function to calculate optimal dropdown position
const calculateDropdownPosition = (inputElement: HTMLInputElement, dropdownMaxHeight: number = 300) => {
    ...
    return {
        ...
        left: `${rect.left}px`,
        width: `${rect.width}px`,
        maxHeight: showAbove ? `${Math.min(spaceAbove - 10, dropdownMaxHeight)}px` : `${Math.min(spaceBelow - 10, dropdownMaxHeight)}px`,
    };
};
...
function GRNCreatePage() {
    ...
    const { data: suppliersData } = useQuery({
        queryKey: ["suppliers"],
        queryFn: () => GrnService.readSuppliers({ skip: 0, limit: 1000, isActive: true }),
    });

    const { data: transportersData } = useQuery({
        queryKey: ["transporters"],
        queryFn: () => GrnService.readTransporters({ skip: 0, limit: 1000, isActive: true }),
    });
    ...
    const { data: filteredTransporters, isLoading: isLoadingTransporters } = useQuery({
    ...
    const { data: productsData, isLoading: isLoadingProducts } = useQuery<ProductsPublic>({
    ...
    const createSupplierMutation = useMutation({
        mutationFn: (data: SupplierCreate) => GrnService.createSupplier({ requestBody: data }),
    ...
    const createTransporterMutation = useMutation({
        mutationFn: (data: TransporterCreate) => GrnService.createTransporter({ requestBody: data }),
    ...
    const createGRNMutation = useMutation({
        mutationFn: (data: GRNCreate) => GrnService.createGrn({ requestBody: data }),
        onSuccess: () => {
    ...
    const getSelectedSupplierName = () => {
        ...
        const supplier = suppliersData?.data.find((s: SupplierPublic) => s.id === formData.supplier_id);
        return supplier?.name || "";
    };
    
    const getSelectedTransporterName = () => {
        if (!formData.transporter_id) return "";
        const transporter = transportersData?.data.find((t: TransporterPublic) => t.id === formData.transporter_id);
    ...
    const handleSubmit = () => {
        ...
        const grnData: GRNCreate = {
            ...
            consignment_date: formData.consignment_date ? new Date(formData.consignment_date).toISOString() : undefined,
                                ...
                                <HStack gap={2}>
                                    <Box flex={1} position="relative">
                                        <Input
                                            ...
                                            size="sm"
                                        />
                                        {showSupplierDropdown && debouncedSupplierSearch.trim().length > 0 && (() => {
                                            ...
                                            return (
                                                <Portal>
                                                    ...
                                                    >
                                                        {isLoadingSuppliers ? (
                                                        ...
                                                        ) : filteredSuppliers?.data && filteredSuppliers.data.length > 0 ? (
                                                            filteredSuppliers.data.map((supplier: SupplierPublic) => (
                                                                <Box
                                                                    key={supplier.id}
                                                                    p={3}
                                                                    cursor="pointer"
                                                                    _hover={{ bg: { base: "gray.700", _light: "gray.100" } }}
                                                                    onClick={() => handleSupplierSelect(supplier)}
                                                                    borderBottomWidth="1px"
                                                                >
                                                                    <Text fontWeight="500" fontSize="sm">{supplier.name}</Text>
                                                                    {supplier.phone && (
                                                            ...
                                                            <Box p={3} textAlign="center">
                                                                <Text fontSize="sm" color="gray.500">No suppliers found</Text>
                                                            </Box>
                                                        )}
                                                    </Box>
                                                </Portal>
                                ...
                                <VStack gap={4} align="stretch">
                                    ...
                                    <Box>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => handleInputChange("consignment_date", e.target.value)}
                                        />
                                    </Box>
                                ...
                                <VStack gap={4} align="stretch">
                                        ...
                                        <HStack gap={2}>
                                            <Box flex={1} position="relative">
                                                <Input
                                                    ...
                                                    value={transporterSearch || getSelectedTransporterName()}
                                                    onChange={(e) => {
                                                        const value = e.target.value;
                                                    ...
                                                    size="sm"
                                                />
                                                {showTransporterDropdown && debouncedTransporterSearch.trim().length > 0 && (() => {
                                                    ...
                                                    return (
                                                        <Portal>
                                                            ...
                                                            >
                                                                {isLoadingTransporters ? (
                                                                ...
                                                                ) : filteredTransporters?.data && filteredTransporters.data.length > 0 ? (
                                                                    filteredTransporters.data.map((transporter: TransporterPublic) => (
                                                                        <Box
                                                                            key={transporter.id}
                                                                            p={3}
                                                                            cursor="pointer"
                                                                            ...
                                                                            borderBottomWidth="1px"
                                                                        >
                                                                            <Text fontWeight="500" fontSize="sm">{transporter.name}</Text>
                                                                            {transporter.phone && (
                                                                                <Text fontSize="xs" color="gray.500">
                                                                    ...
                                                                    <Box p={3} textAlign="center">
                                                                        <Text fontSize="sm" color="gray.500">No transporters found</Text>
                                                                    </Box>
                                                                )}
                                                            </Box>
                                                        </Portal>
                                    ...
                                    <Box>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => handleInputChange("vehicle_reg_number", e.target.value)}
                            ...
                            <Box overflowX="auto">
                                <Table.Root size="sm" variant="outline">
                                    ...
                                    <Table.Body>
                                        {items.length === 0 ? (
                                            <Table.Row>
                                                <Table.Cell colSpan={7} textAlign="center" color="gray.500" py={8}>
                                                ...
                                                <Table.Row key={item.id}>
                                                    <Table.Cell>
                                                        <Input
                                                            ...
                                                            onChange={(e) => handleItemChange(index, "lpo_number", e.target.value)}
                                                            placeholder="LPO"
                                                        />
                                                    </Table.Cell>
                                                    <Table.Cell minW="250px">
                                                        <Box position="relative">
                                                            ...
                                                            />
                                                            {activeItemIndex === index && debouncedProductSearch.trim().length > 0 && (() => {
                                                                    ...
                                                                    <Portal>
                                                                        ...
                                                                        >
                                                                            {isLoadingProducts ? (
                                                                                ...
                                                                                productsData.data.map((product: ProductPublic) => (
                                                                                    <Box
                                                                                        key={product.id}
                                                                                        p={3}
                                                                                        cursor="pointer"
                                                                                        ...
                                                                                        borderBottomWidth="1px"
                                                                                    >
                                                                                        <Text fontWeight="500" fontSize="sm">{product.name}</Text>
                                                                                        <Text fontSize="xs" color="gray.500">
                                                    ...
                                                    <Table.Cell>
                                                        <Input
                                                            ...
                                                            onChange={(e) => handleItemChange(index, "order_quantity", parseFloat(e.target.value) || 0)}
                                                            placeholder="0"
                                                        />
                                                    </Table.Cell>
                                                    <Table.Cell>
                                                        <Input
                                                            ...
                                                            onChange={(e) => handleItemChange(index, "pending_quantity", parseFloat(e.target.value) || 0)}
                                                            placeholder="0"
                                                        />
                                                    </Table.Cell>
                                                    <Table.Cell>
                                                        <Input
                                                            ...
                                                            placeholder="0"
                                                            borderColor={item.received_quantity === 0 ? "red.500" : undefined}
                                                        />
                                                    </Table.Cell>
                                                    <Table.Cell textAlign="center">
                                                        <input
                                                            ...
                                                            onChange={(e) => handleItemChange(index, "is_promo", e.target.checked)}
                                                        />
                                                    </Table.Cell>
                                                    <Table.Cell>
                                                        <IconButton

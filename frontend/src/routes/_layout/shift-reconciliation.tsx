...
import { Box, Container, Heading, Text, Flex, HStack, VStack, Input, Button, Icon, Textarea, Link } from "@chakra-ui/react"
import { useState, useEffect, useMemo } from "react"
import { ThemedSelect } from "@/components/POS/ThemedSelect"
import { formatCurrency } from "@/components/POS/utils"
import { FiSave, FiLock, FiUnlock, FiCalendar } from "react-icons/fi"
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query"
import useCustomToast from "@/hooks/useCustomToast"
import useAuth from "@/hooks/useAuth"
import { Checkbox } from "@/components/ui/checkbox"
import { DialogRoot, DialogContent, DialogHeader, DialogTitle, DialogBody, DialogFooter, DialogTrigger, DialogActionTrigger, DialogCloseTrigger } from "@/components/ui/dialog"
...
function ShiftReconciliation() {
  ...
  const [transferMpesaChecked, setTransferMpesaChecked] = useState<boolean>(false)
  ...
  const { data: lastClosedShiftData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`${API_BASE}/api/v1/till?status=closed&limit=1`, {
        headers: getAuthHeaders(),
      })
      if (!response.ok) {
        return null
  ...
  const { data: closedTillsData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`${API_BASE}/api/v1/till?status=closed&limit=100`, {
        headers: getAuthHeaders(),
      })
      if (!response.ok) {
        return { data: [], count: 0 }
  ...
  const { data: salesData } = useQuery({
    ...
    queryFn: async () => {
      if (!currentTill) return { total_sales: 0, cash_sales: 0, mpesa_sales: 0, pdq_sales: 0, equity_sales: 0, cheque_sales: 0, credit_notes: 0 }
      
      const startDate = new Date(currentTill.opening_time)
      const endDate = currentTill.closing_time ? new Date(currentTill.closing_time) : new Date()
      
      const startDateStr = startDate.toISOString().split('T')[0]
      const endDateStr = endDate.toISOString().split('T')[0]
      
      const response = await fetch(`${API_BASE}/api/v1/sales?start_date=${startDateStr}&end_date=${endDateStr}&limit=1000`, {
        headers: getAuthHeaders(),
      })
      if (!response.ok) {
        return { total_sales: 0, cash_sales: 0, mpesa_sales: 0, pdq_sales: 0, equity_sales: 0, cheque_sales: 0, credit_notes: 0 }
      }
      const data = await response.json()
      ...
      sales.forEach((sale: any) => {
        ...
        if (saleDate < startDate || (currentTill.closing_time && saleDate > endDate)) {
          return
        }
    ...
    mutationFn: async () => {
      const response = await fetch(`${API_BASE}/api/v1/till/open`, {
        ...
        body: JSON.stringify({
          ...
          opening_balance: openingBalance && parseFloat(openingBalance) > 0 ? parseFloat(openingBalance) : undefined,
  ...
  const closeTillMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`${API_BASE}/api/v1/till/close?closing_cash_float=${parseFloat(closingCashFloat) || 0}`, {
        method: "POST",
        headers: getAuthHeaders(),
      })
      if (!response.ok) {
  ...
  const reconcileMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (varianceType === "none") {
        showToast.showSuccessToast("Shift reconciled successfully - No variance")
      } else {
  ...
  // Calculate totals
  const grandTotalSales = useMemo(() => salesData?.total_sales || 0, [salesData])
  const totalPosCNotes = useMemo(() => salesData?.credit_notes || 0, [salesData])
  const totalNetSales = useMemo(() => grandTotalSales - totalPosCNotes, [grandTotalSales, totalPosCNotes])
  const totalSalesPaid = useMemo(() => {
    return (salesData?.pdq_sales || 0) + (salesData?.equity_sales || 0) + (salesData?.cheque_sales || 0)
  }, [salesData])
  ...
  const totalMpesaBal = mpesaOpeningBal + mpesaPosSales + othMpesaSales - mpesaExpenses
  const mpesaAtHandNum = parseFloat(mpesaAtHand || "0")
  const transferMpesaBalNum = parseFloat(transferMpesaBal || "0")
  const mpesaClosingBal = mpesaAtHandNum - (transferMpesaChecked ? transferMpesaBalNum : 0)
  ...
  const handleSubmit = () => {
    if (task === "open") {
      ...
      if (lastClosedShift && lastClosedShift.status !== "reconciled" && (!openingBalance || parseFloat(openingBalance) <= 0)) {
      ...
      if (!systemCounts?.payment_methods || systemCounts.payment_methods.length === 0) {
      ...
      <Flex gap={4} flexDirection={{ base: "column", lg: "row" }}>
          ...
          <VStack gap={2} align="stretch">
            ...
            <Box>
              <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                Task <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
              </Text>
              <ThemedSelect
                value={task}
            ...
            <Box>
              <Text fontSize="sm" fontWeight="medium" mb={1} color={{ base: "#06b6d4", _light: "#06b6d4" }}>
                {task === "open" ? "Attach Closed Till" : "Attach Open Till"} <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
              </Text>
              {task === "open" ? (
                <>
                  {attach && closedTillsData?.data?.find((till) => till.id === attach) ? (
                    <Box
                      p={2}
                      border="1px solid"
                    ...
                    >
                      <Text fontSize="sm" color="text.primary">
                        {(() => {
                          const selectedTill = closedTillsData.data.find((till) => till.id === attach)!
                          ...
                          const shiftType = selectedTill.shift_type === "day" ? "Day" : "Night"
                          const cashierName = selectedTill.opened_by_name || "Unknown"
                          const status = selectedTill.status === "reconciled" ? "Reconciled" : "Closed"
                    ...
                    >
                      <DialogTrigger asChild>
                        <Link
                          ...
                          cursor="pointer"
                          fontSize="sm"
                          _hover={{ color: { base: "#0891b2", _light: "#0891b2" } }}
                        >
                      ...
                      <VStack gap={2} align="stretch" maxH="400px" overflowY="auto">
                        {closedTillsData?.data && closedTillsData.data.length > 0 ? (
                          closedTillsData.data.map((till) => {
                            ...
                            const cashierName = till.opened_by_name || "Unknown"
                            const status = till.status === "reconciled" ? "Reconciled" : "Closed"
                            ...
                            return (
                              <Box
                                ...
                                p={3}
                                border="1px solid"
                                borderColor={isSelected ? { base: "#06b6d4", _light: "#06b6d4" } : "border.card"}
                                borderRadius="md"
                                bg={isSelected ? { base: "rgba(6, 182, 212, 0.1)", _light: "rgba(6, 182, 212, 0.05)" } : "bg.surface"}
                                cursor="pointer"
                                onClick={() => {
                                  setAttach(till.id)
                                  // Update opening balance when a closed till is selected
                                  if (till.closing_cash_float) {
                                    setOpeningBalance(till.closing_cash_float.toString())
                                  }
                                  // Determine next shift type
                                  const nextShift = till.shift_type === "day" ? "night" : "day"
                                  setShiftType(nextShift as "day" | "night")
                                  setWorkingShift(nextShift === "day" ? "Morning Shift" : "Evening Shift")
                                  setIsAttachModalOpen(false)
                                }}
                                _hover={{ bg: { base: "rgba(6, 182, 212, 0.1)", _light: "rgba(6, 182, 212, 0.05)" } }}
                              >
                                <Text fontSize="sm" fontWeight={isSelected ? "bold" : "medium"} color="text.primary">
                                ...
                                {till.closing_cash_float && (
                                  <Text fontSize="xs" color="text.secondary">
                                    Closing Balance: {formatCurrency(till.closing_cash_float)}
                                  </Text>
                                )}
                              </Box>
                            )
                    ...
                    >
                      <Text fontSize="sm" color="text.primary">
                        {(() => {
                          ...
                          const dateStr = openDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                          const timeStr = openDate.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })
                          const shiftType = currentTill.shift_type === "day" ? "Day" : "Night"
                          const cashierName = currentTill.opened_by_name || "Unknown"
            ...
            {task === "open" && (
              <Box>
                <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                  Till Cash Account <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
                </Text>
                <Input
                  ...
                  color="text.primary"
                  _focus={{ borderColor: { base: "#14b8a6", _light: "#14b8a6" }, boxShadow: { base: "0 0 0 1px #14b8a6", _light: "0 0 0 1px #14b8a6" } }}
                />
              </Box>
            )}
            ...
            <Box>
              <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                Till Cashier <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
              </Text>
              <Input
                value={tillCashier}
            ...
            <Box>
              <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                Till Supervisor <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
              </Text>
              <Input
            ...
            <Box>
              <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                Working Shift <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
              </Text>
              <ThemedSelect
                value={workingShift}
                onChange={(value) => {
            ...
            <Box>
              <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                Create Date <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
              </Text>
              <Input
                value={currentDate}
                readOnly
            ...
            {task === "close" && (
              <Box>
                <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                  Close Date <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
                </Text>
                <HStack gap={2}>
                  <Input
                    value={closeDate}
                ...
                <HStack gap={2} mt={2}>
                  <Checkbox
                    ...
                    onCheckedChange={({ checked }) => setCheckApprove(checked as boolean)}
                  />
                  <Text fontSize="sm" color="text.primary">Check</Text>
                </HStack>
              </Box>
            )}
            ...
            {task === "open" && (
              <Box>
                <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                  ...
                  <Text as="span" color={lastClosedShift && lastClosedShift.status !== "reconciled" ? { base: "#ef4444", _light: "#ef4444" } : "text.secondary"}>
                    {lastClosedShift && lastClosedShift.status !== "reconciled" ? "(Required)" : "(Optional)"}
                  </Text>
                </Text>
                <Input
                  type="number"
                  ...
                  bg="bg.surface"
                  borderColor={
                    lastClosedShift && lastClosedShift.status !== "reconciled" && (!openingBalance || parseFloat(openingBalance) <= 0)
            ...
            {task === "open" && (
              <Box>
                <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                  Opening Cash Float <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
                </Text>
                <Input
                  type="number"
                  step="0.01"
            ...
            {task === "close" && (
              <Box>
                <Text fontSize="sm" fontWeight="medium" mb={1} color="text.primary">
                  Closing Cash Float <Text as="span" color={{ base: "#ef4444", _light: "#ef4444" }}>*</Text>
                </Text>
                <Input
                  type="number"
                  step="0.01"
            ...
            {systemCounts && task !== "open" && task !== "close" && (
              <>
                {systemCounts.payment_methods.map((pm) => {
                  const isCash = pm.payment_method_name.toUpperCase().includes("CASH")
                  const isMpesa = pm.payment_method_name.toUpperCase().includes("MPESA")
            ...
            {/* GRAND TOTAL Section */}
            <Box mt={2} p={4} bg={{ base: "rgba(255, 255, 255, 0.05)", _light: "#f9fafb" }} borderRadius="md">
              <Text fontSize="sm" fontWeight="bold" mb={2} color="text.primary">
                GRAND TOTAL
              </Text>
              <VStack gap={1} align="stretch">
                <HStack justify="space-between">
                  <Text fontSize="sm" color="text.primary">GRAND TOTAL SALES</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                    {formatCurrency(grandTotalSales)}
                  </Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontSize="sm" color="text.primary">TOTAL POS CNOTES</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                    {formatCurrency(totalPosCNotes)}
                  </Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontSize="sm" color="text.primary">TOTAL NET SALES</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
            ...
            <Box mt={2} p={4} bg={{ base: "rgba(255, 255, 255, 0.05)", _light: "#f9fafb" }} borderRadius="md">
              ...
              <VStack gap={1} align="stretch">
                <HStack justify="space-between">
                  <Text fontSize="sm" color="text.primary">PDQ Sales</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                    {formatCurrency(salesData?.pdq_sales || 0)}
                  </Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontSize="sm" color="text.primary">Equity Sales</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                    {formatCurrency(salesData?.equity_sales || 0)}
                  </Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontSize="sm" color="text.primary">Cheque Sales</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                    {formatCurrency(salesData?.cheque_sales || 0)}
                  </Text>
                </HStack>
                <HStack justify="space-between">
                  <Text fontSize="sm" fontWeight="bold" color="text.primary">TOTAL SALES PAID</Text>
                  <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
            ...
            <Button
              ...
              loading={openTillMutation.isPending || closeTillMutation.isPending || reconcileMutation.isPending}
              disabled={
                (task === "open" && (!openingCashFloat || parseFloat(openingCashFloat) < 0)) ||
                (task === "close" && (!closingCashFloat || parseFloat(closingCashFloat) < 0))
              }
              w="full"
              mt={2}
            >
              <Icon as={task === "open" ? FiUnlock : task === "close" ? FiLock : FiSave} mr={2} />
              {task === "open" ? "Open" : task === "close" ? "Close" : "Reconcile"}
            </Button>
          </VStack>
        </Box>
        ...
        <Box flex="1" bg="bg.surface" borderRadius="lg" p={6} border="1px solid" borderColor="border.card">
          ...
          {task === "open" ? (
            <VStack gap={2} align="stretch">
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Cash Opening Bal</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(openingBalanceNum)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Mpesa Opening Bal</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
            ...
            <VStack gap={2} align="stretch">
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Opening Balance</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(openingBalanceNum)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">POS Cash Sales</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(posCashSales)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Till Cbk Rcts</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(tillCbkRcts)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Till Cbk Expenses</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(tillCbkExpenses)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color={{ base: "#ef4444", _light: "#ef4444" }}>Pending Cbk Rcts</Text>
                <Text fontSize="sm" fontWeight="bold" color={{ base: "#ef4444", _light: "#ef4444" }} textAlign="right">
                  {formatCurrency(pendingCbkRcts)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color={{ base: "#ef4444", _light: "#ef4444" }}>Pending Cbk Pymts</Text>
                <Text fontSize="sm" fontWeight="bold" color={{ base: "#ef4444", _light: "#ef4444" }} textAlign="right">
                  {formatCurrency(pendingCbkPymts)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Money In</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(moneyIn)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Money Out</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(moneyOut)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color={{ base: "#3b82f6", _light: "#3b82f6" }} fontWeight="bold">Expected Till Balance</Text>
                <Text fontSize="sm" fontWeight="bold" color={{ base: "#3b82f6", _light: "#3b82f6" }} textAlign="right">
                  {formatCurrency(expectedTillBalance)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Physical Cash</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(physicalCashNum)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Cash Closing Bal</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(cashClosingBal)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Till Total Sales</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(tillTotalSales)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Till POS CNotes</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(tillPosCNotes)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Till Net Sales</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
        ...
        {task === "close" && (
          <Box flex="1" bg="bg.surface" borderRadius="lg" p={6} border="1px solid" borderColor="border.card">
            ...
            <VStack gap={2} align="stretch">
              ...
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Mpesa POS Sales</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(mpesaPosSales)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Oth.Mpesa Sales</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(othMpesaSales)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Mpesa Expenses</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(mpesaExpenses)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color="text.primary">Total Mpesa Bal</Text>
                <Text fontSize="sm" fontWeight="bold" color="text.primary" textAlign="right">
                  {formatCurrency(totalMpesaBal)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color={{ base: "#10b981", _light: "#10b981" }} fontWeight="bold">MPESA (At Hand)</Text>
                <Text fontSize="sm" fontWeight="bold" color={{ base: "#10b981", _light: "#10b981" }} textAlign="right">
              ...
              <HStack gap={2} align="center" justify="space-between">
                <HStack gap={2}>
                  <Checkbox
                    ...
                    onCheckedChange={({ checked }) => setTransferMpesaChecked(checked as boolean)}
                  />
                  <Text fontSize="sm" color="text.primary">TransferMpesaBal</Text>
                </HStack>
                <Input
                  type="number"
                  step="0.01"
              ...
              <HStack justify="space-between">
                <Text fontSize="sm" color={{ base: "#10b981", _light: "#10b981" }} fontWeight="bold">MPESA Closing Bal</Text>
                <Text fontSize="sm" fontWeight="bold" color={{ base: "#10b981", _light: "#10b981" }} textAlign="right">
                  {formatCurrency(mpesaClosingBal)}
                </Text>
              </HStack>
              <HStack justify="space-between">
                <Text fontSize="sm" color={{ base: "#ef4444", _light: "#ef4444" }} fontWeight="bold">MPESA Not Validated</Text>
                <Text fontSize="sm" fontWeight="bold" color={{ base: "#ef4444", _light: "#ef4444" }} textAlign="right">

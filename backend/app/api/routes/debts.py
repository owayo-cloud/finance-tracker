...
def create_debt(
    *, session: SessionDep, current_user: CurrentUser, debt_in: DebtCreate
) -> Any:
    """
    ...
    try:
        ...
        logger.error(
            f"Error creating debt: {type(e).__name__}: {str(e)}", exc_info=True
        )
        session.rollback()
...
def delete_debt(
    *, session: SessionDep, admin_user: AdminUser, debt_id: uuid.UUID
) -> Any:
    """
...
def create_debt_payment(
    *,
    ...
    payment = DebtPayment(
        **payment_in.model_dump(), debt_id=debt_id, created_by_id=current_user.id
    )
    session.add(payment)
...
def get_customer_balance(
    *, session: SessionDep, current_user: CurrentUser, customer_name: str
) -> Any:
    """
    Get total outstanding balance for a customer.
    """
    statement = select(func.sum(Debt.balance)).where(
        and_(Debt.customer_name == customer_name, Debt.balance > 0)
    )

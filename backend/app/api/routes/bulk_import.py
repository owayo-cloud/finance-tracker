...
def auto_map_columns(uploaded_columns: list[str]) -> dict[str, str]:
    ...
    mapping_rules = {
        ...
        "buying_price": ["buying price", "cost price", "buy price", "purchase price", "bp", "cost"],
...
def validate_row_data(
    ...
    if not row_data.get("selling_price"):
        ...
        try:
            selling_price = Decimal(str(row_data["selling_price"]).strip().replace(",", ""))
            if selling_price <= 0:
                errors.append(ValidationError(
                    field="selling_price",
    ...
    if row_data.get("buying_price"):
        try:
            buying_price = Decimal(str(row_data["buying_price"]).strip().replace(",", ""))
            if buying_price < 0:
                errors.append(ValidationError(
                    field="buying_price",
                ...
                if "selling_price" in mapped_data and buying_price > mapped_data["selling_price"]:
                    warnings.append(
...
def generate_csv_template() -> str:
    ...
    template_data = [
        ["Product Name", "Selling Price", "Buying Price", "Product Category", "Description", "Current Stock", "Reorder Level"],
        ["2 Share Sweet Red 750ML", "850.00", "650.00", "Wine", "Sweet red wine 750ml bottle", "24", "10"],
        ["2 Share Sweet White 750ML", "850.00", "650.00", "Wine", "Sweet white wine 750ml bottle", "18", "10"],
        ["Coca Cola 300ML", "120.00", "80.00", "Beverages", "Coca Cola 300ml bottle", "48", "20"],
        ["Tusker Lager 500ML", "200.00", "150.00", "Beer", "Tusker Lager beer 500ml bottle", "36", "15"],
    ]
...
def map_columns(
    *,
    ...
    session_data["default_category_id"] = str(request.default_category_id) if request.default_category_id else None
    session_data["default_status_id"] = str(request.default_status_id) if request.default_status_id else None
...
def get_validation_results(
    *,
    ...
    skip: int = 0,
    limit: int = 100,
    filter: Optional[str] = None,  # all, errors, duplicates, warnings
) -> Any:
    """
    ...
    if filter == "errors":
        ...
        filtered_rows = [r for r in validated_rows if r.status == ImportRowStatus.DUPLICATE]
    elif filter == "warnings":
        filtered_rows = [r for r in validated_rows if r.status == ImportRowStatus.WARNING]
    else:
    ...
    # Count by status
    valid_count = sum(1 for r in validated_rows if r.status in [ImportRowStatus.VALID, ImportRowStatus.WARNING])
    error_count = sum(1 for r in validated_rows if r.status == ImportRowStatus.ERROR)
    duplicate_count = sum(1 for r in validated_rows if r.status == ImportRowStatus.DUPLICATE)
        ...
        try:
            if action == "create":
                ...
                if existing_product:
                    ...
                    existing_product.buying_price = row.mapped_data.get("buying_price", Decimal("0.00"))
                    existing_product.current_stock = row.mapped_data.get("current_stock", 0)
                    if row.mapped_data.get("reorder_level") is not None:
                        existing_product.reorder_level = row.mapped_data["reorder_level"]

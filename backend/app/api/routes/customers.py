...
def get_customers(
    ...
    search: str | None = Query(None, description="Search by name or tel"),
    ...
    last_sale_query = (
        select(Sale.customer_name, func.max(Sale.sale_date).label("last_sale_date"))
    ...
    debt_contacts_query = (
        select(Debt.customer_name, Debt.customer_contact)
        .where(
            and_(
    ...
    contact_map: dict[str, str] = {
        row.customer_name.lower().strip(): row.customer_contact
        for row in debt_contacts
    ...
    sale_map: dict[str, datetime] = {
        row.customer_name.lower().strip(): row.last_sale_date
    ...
    for key, last_sale_date in sale_map.items():
        if key in customer_map:
            ...
            if last_sale_date and (
                not existing.last_sale_date
                or last_sale_date
                > datetime.fromisoformat(existing.last_sale_date.replace("Z", "+00:00"))
            ):
            ...
            original_name = next(
                (
                    row.customer_name
                    for row in sale_results  # type: ignore[attr-defined]

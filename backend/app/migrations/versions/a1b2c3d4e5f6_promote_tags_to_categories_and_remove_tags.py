...
def upgrade() -> None:
    ...
    tags_result = connection.execute(sa.text("SELECT id, name FROM product_tag"))
    ...
    for tag_row in tags_result:
        ...
        for category_name, cat_id in tag_to_category_map.items():
            if (
                category_name.lower() == tag_name.lower()
                or category_name.lower().replace(" ", "-")
                == tag_name.lower().replace(" ", "-")
            ):
...
def downgrade() -> None:
    ...
    op.create_foreign_key(
        "product_tag_id_fkey", "product", "product_tag", ["tag_id"], ["id"]
    )
